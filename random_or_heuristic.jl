...
for i in 1:6
    ...
    for i in 1:m
        for j in 1:n
            if data[i,j] != 0.
                data[i,j] += rand(Uniform(-0.1*data[i,j], 0.1*data[i,j]))
            end
        end
    end
    linear(x) = x
    ...
    for j in 1:50
        ...
        lsq_am(p) = adams_moulton_estimator(p, data, p_solve.t, p_solve.fun)
        ...
        res_lsf_am = lsf.curve_fit(lsq_am_curvefit, p_solve.t, vec(data), p0, lower=p_solve.bounds[1], upper=p_solve.bounds[end])
        res_lsf_am_dist = euclidean(p_solve.phi, res_lsf_am.param)
        push!(res_lsf_am_dist_array, res_lsf_am_dist)
        #p0 = res_lsf_am.param

        lsq_am_sum(p) = sum(abs2.(loss.(adams_moulton_estimator(p, data, p_solve.t, p_solve.fun))))
        ...
        lsq_ss(p) = vec(single_shooting_estimator(p, data, p_solve.t, p_solve.fun))
