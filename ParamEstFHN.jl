...
@time (minf,minx,ret) = NLopt.optimize(opt,glo_init)
println("\nObjective function:\t$minf\nFor parameters:\n$minx\nReal parameters are:\n$p\nMessage:\t$ret")
# Very Good - Improved Speed and Estimation
...
obj_am(x,y) = sum(abs2.(adams_moulton_estimator(x, data, t, fitzHugh)))
obj_am_g(x) = sum(abs2.(adams_moulton_estimator(x, data, t, fitzHugh)))
